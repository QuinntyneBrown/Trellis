...
/**
 * The two "Explain This" endpoints. Local selections are read client-side
 * (File System Access API) and posted as path+content pairs; repository URLs
 * are fetched and aggregated entirely server-side -- either way the backend
 * owns the one implementation of the GetFiles-style aggregation and the
 * prompt text.
 */
@Injectable({
  providedIn: 'root',
